...
export class MyCameraPointersInput implements BABYLON.ICameraInput<MyCamera> {
    ...
    private pointerMode: number = MyCameraPointersInput.POINTER_MODE_SELECT;
    public static POINTER_MODE_SELECT: number = 1;
    public static POINTER_MODE_PAN: number = 2;
    ...
    public attachControl(element: HTMLElement, noPreventDefault?: boolean) {
        ...
        this._pointerInput = (p, s) => {
            ...
            if (p.type === BABYLON.PointerEventTypes.POINTERDOWN) {
                ...
                var panMode = (this.pointerMode == MyCameraPointersInput.POINTER_MODE_PAN);

                // One button down
                if (pointA && pointB === undefined) {
                    if (this.panningSensibility !== 0 &&

                        ((   (this._isCtrlPushed || panMode) && this.camera._useCtrlForPanning) ||
                        (!this.camera._useCtrlForPanning && this._isRightClick) || panMode)
                    )
...
BABYLON.CameraInputTypes["MyCameraPointersInput"] = MyCameraPointersInput;
